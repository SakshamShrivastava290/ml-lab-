"""
=============================================================
  ADABOOST (BOOSTING) — Standalone Lab Program
=============================================================
  Supports: Classification AND Regression
  Libraries: pandas, numpy, matplotlib, seaborn, sklearn
  Usage: python boosting.py
=============================================================
"""
...
def run():
    ...
    n = input("Number of estimators (default 50): ").strip()
    n = int(n) if n else 50
    if mode == 'r':
        model = AdaBoostRegressor(n_estimators=n, random_state=42)
        model.fit(X_train, y_train)
        evaluate_regression(y_test, model.predict(X_test), f"AdaBoost Regressor (n={n})")
    else:
        model = AdaBoostClassifier(n_estimators=n, random_state=42, algorithm='SAMME')
        model.fit(X_train, y_train)
        evaluate_classification(y_test, model.predict(X_test), f"AdaBoost Classifier (n={n})")
